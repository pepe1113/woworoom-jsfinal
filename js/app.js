...
const orderInputs = document.querySelectorAll('input[type=text],input[type=tel],input[type=email]')
...
function toRenderCartStr(item, index) {
  ...
  carts.innerHTML += `<tr>
    ...
    <td>
    <a href="#">
    <span class="material-icons remove" data-num="${item.quantity - 1}" data-id="${item.id}">remove</span></a>
    <span>${item.quantity}</span>
    <a href="#">
    <span class="material-icons add" data-num="${item.quantity + 1}" data-id="${item.id}">add</span></a>
    </td>
    <td>NT$${total}</td>
    <td class="discardBtn">
...
function addCartList(e) {
  if (e.target.getAttribute('id') === 'addCardBtn') {
    ...
    let alreadyAddArr = cartList.filter((i) => i.product.id === obj.data.productId)
...
function ValueErrorCheck() {
  orderInputs.forEach(function (item) {
    item.addEventListener('blur', function (e) {
      ...
      item.nextElementSibling.textContent = errors[e.target.getAttribute('name')]
    })
  })
}
